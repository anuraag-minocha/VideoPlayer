...
class ExoPlayerRecyclerView : RecyclerView {
    ...
    // vars
    private var mediaObjects: ArrayList<File> = ArrayList()
    ...
    fun playVideo(isEndOfList: Boolean) {
        ...
        val holder: MainRecyclerAdapter.MainViewHolder =
            child.tag as MainRecyclerAdapter.MainViewHolder
        if (holder == null) {
            playPosition = -1
            return
        }
        ...
        val mediaUrl: File = mediaObjects[targetPosition]
        if (mediaUrl != null) {
            val videoSource: MediaSource = ExtractorMediaSource.Factory(dataSourceFactory)
                .createMediaSource(Uri.fromFile(mediaUrl))
    ...
    fun setMediaObjects(mediaObjects: ArrayList<File>) {
